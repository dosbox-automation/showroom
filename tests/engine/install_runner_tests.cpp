...
TEST_F(InstallRunnerFixture, a_downloaded_disk_image_mounts_in_place_and_never_extracts)
{
    // ckeen4's pinned source IS the floppy; handing it to the extractor
    // fails with "Unrecognized archive format" (aug-p0kd).
    std::ofstream(cache_ / "downloads" / "doom" / "game.img") << "image bytes";
    server_.queueScriptStatus(
            R"({"state":"completed","output":{"install_complete":"yes"}})");

    QSignalSpy succeeded(runner_.get(), &InstallRunner::succeeded);
    std::string error;
    ASSERT_TRUE(runner_->startInstall(installableGame(30, "floppyinstall", "game.img"),
                                      error))
            << error;
    plantStagedResult();
    ...
    EXPECT_EQ(extractor_.calls, 0);
    EXPECT_TRUE(fs::is_regular_file(cache_ / "install.conf"));
    EXPECT_TRUE(fs::is_regular_file(cache_ / "downloads" / "doom" / "game.img"));
}
